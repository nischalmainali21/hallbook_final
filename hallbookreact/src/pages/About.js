...
export default function About() {
  ...
  let rejectBooking = async (id, payload) => {
    try {
      let response = await fetch(
        `http://127.0.0.1:8000/api/hall/bookings/${id}/`,
        {
  ...
  const handleRejectClick = (
    ...
    const payload = new FormData();
    payload.append("bookedHall", bookedHallID);
    payload.append("rejected", true); //!important
    payload.append("event", eventID);
    payload.append("startTime", startTime);
    payload.append("endTime", endTime);
    payload.append("eventDate", eventDate);
  ...
  console.log(data);
  let pendingData = data.filter((item) => {return !item.verified&&!item.rejected});
  ...
  return (
    <div className="mx-auto mt-10 flex min-h-screen max-w-4xl flex-col gap-12 rounded-lg p-2 shadow-lg md:w-2/3 md:gap-8">
      ...
      {!rejectedData.length?(<div className="text-center font-bold text-cprimary-600">
          No Rejected Bookings
        </div>):(rejectedData.map(item=>(
          <FacultyBookingCard
          key={item.id}
          id={item.id}
          eventID={item.event}
